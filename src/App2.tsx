...
const App = () => {
  ...
  const API_KEY = process.env.REACT_APP_GOOGLE_PROJECT_API_KEY;

  useEffect(() => {
    // const fetchAllFiles = async (folderId: string): Promise<FileOrFolder[]> => {
    //   try {
    //     const endpoint = `https://www.googleapis.com/drive/v3/files?q='${folderId}'+in+parents&key=${API_KEY}&fields=files(id,name,mimeType,parents)`;
    ...
    //     const folderNodes = files || [];

    //     // Fetch children for each subfolder recursively
    //     const subfolderPromises = folderNodes
    //       .filter(
    //         (item: any) =>
    //           item.mimeType === "application/vnd.google-apps.folder"
    //       )
    //       .map(async (folder: any) => {
    //         const children = await fetchAllFiles(folder.id);
    //         return { ...folder, children }; // Attach children to folder node
    //       });

    //     const resolvedFolders = await Promise.all(subfolderPromises);

    //     // Combine resolved folders with non-folder files
    //     return folderNodes.map((item: any) =>
    //       item.mimeType === "application/vnd.google-apps.folder"
    //         ? resolvedFolders.find((folder) => folder.id === item.id) || item
    //         : item
    //     );
    //   } catch (err) {
...
const fetchAllFiles = async (folderId: string): Promise<FileOrFolder[]> => {
  try {
    ...
    // Map through files and add the correct download URL for images
    const fileWithUrls = folderNodes.map((item: any) => {
      if (item.mimeType.startsWith("image/")) {
        item.downloadUrl = `https://drive.google.com/uc?export=download&id=${item.id}`;
      }
      return item;
    });

    // Combine resolved folders with non-folder files
    return fileWithUrls.map((item: any) =>
      item.mimeType === "application/vnd.google-apps.folder"
        ? resolvedFolders.find((folder) => folder.id === item.id) || item
        : item
    );
    ...
    const constructHierarchy = async () => {
      const rootStructure = {
        ...
        children: await fetchAllFiles(FOLDER_ID || ""),
      };
      // const tree = JSON.stringify(rootStructure, null, 2)
      // console.log("Folder Structure:", tree);
      setGoogleDrive(rootStructure)
    };
